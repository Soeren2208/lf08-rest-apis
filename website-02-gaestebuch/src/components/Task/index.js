 ...
 *
 * Der Haken-Zustand wird unter dem Key `task:gaestebuch:${id}` in
 * localStorage persistiert. Das Präfix `gaestebuch` trennt den Fortschritt
 * dieses (zweiten) Tutorials von dem des ersten Tutorials (Personenverwaltung),
 * falls beide Seiten jemals unter derselben Origin ausgeliefert werden.
 */
export default function Task({id, children}) {
  ...
  const storageKey = `task:gaestebuch:${id}`;
