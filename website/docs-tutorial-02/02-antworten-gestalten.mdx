...
:::info Neu: `.map(ResponseEntity::ok)`
`findById` liefert ein `Optional`. Mit `map` wird der Inhalt umgewandelt, **falls** einer da ist: Aus `Optional<GuestbookEntry>` wird `Optional<ResponseEntity<GuestbookEntry>>`.

Ist nichts da, greift `orElseThrow` — und Spring macht daraus die Antwort `404`.

Die Schreibweise `ResponseEntity::ok` ist eine **Methodenreferenz**. Sie ist die Kurzform von `entry -> ResponseEntity.ok(entry)`.
:::
...
Deine API antwortet jetzt in fünf verschiedenen Situationen. Ordne zu:

<Task id="t2-ab02-a3">Trage in die Tabelle ein, welchen Statuscode dein Dienst liefert — und prüfe es anschließend durch Ausprobieren.</Task>

| Situation | Dein Statuscode |
|---|---|
...
|---|---|
| Eintrag erfolgreich angelegt | `201 Created` |
| Alle Einträge abgerufen | `200 OK` |
| Eintrag mit unbekannter Id abgerufen | `404 Not Found` |
| Eintrag erfolgreich gelöscht | `204 No Content` |
| Eintrag mit unbekannter Id gelöscht | `404 Not Found` |
| JSON im Rumpf war fehlerhaft | `400 Bad Request` |
| `DELETE` auf die Sammlung | `405 Method Not Allowed` |
